...
namespace Tsp
{
    class Gsp
    {
        public delegate void NewBestTourEventHandler(Object sender, GspEventArgs e);
        ...
        public void Begin(int populationSize, int maxGenerations, int groupSize, int mutation, int seed, int chanceToUseCloseCity, Sehirler cityList)
        {
            rand = new Random(seed);

            this.cityList = cityList;

            population = new Populasyon();
        ...
        bool makeChildren(int groupSize, int mutation)
        {
            ...
            population[childPosition] = Tur.Crossover(population[tourGroup[0]], population[tourGroup[1]], cityList, rand);
            ...
            population[childPosition] = Tur.Crossover(population[tourGroup[1]], population[tourGroup[0]], cityList, rand);
            if (rand.Next(100) < mutation)
            {
                population[childPosition].Mutate(rand);
            }
        ...
        void displayTour(Tur bestTour, int generationNumber, bool complete)
        {
            if (foundNewBestTour != null)
            {
                this.foundNewBestTour(this, new GspEventArgs(cityList, bestTour, generationNumber, complete));
            }
        }
    }
}
